...
config_after_install() {
    ...
    echo -e "${green}账号: ${config_account}${plain}"
    echo -e "${green}密码: ${config_password}${plain}"
    echo -e "${green}端口: ${config_port}${plain}"
    
    /dada/x-ui/x-ui setting -username ${config_account} -password ${config_password}
    /dada/x-ui/x-ui setting -port ${config_port}
    
    echo -e "${green}自动配置已完成${plain}"
}
...
install_x-ui() {
    ...
    cd /dada
    tar zxvf /usr/local/x-ui-linux-${arch}.tar.gz
    rm /usr/local/x-ui-linux-${arch}.tar.gz -f
    cd x-ui
    
    # 修改服务文件路径
    sed -i 's|/usr/local/x-ui|/dada/x-ui|g' x-ui.service
    ...
    sed -i 's|/usr/local/x-ui|/dada/x-ui|g' /usr/bin/x-ui
    
    chmod +x /dada/x-ui/x-ui.sh
